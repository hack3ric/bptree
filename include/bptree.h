...
typedef struct bptree_node {
  ...
  union {
    ...
    struct bptree_node *children[ORDER]; // For internal nodes
  };
  struct bptree_node *next; // For leaf nodes to form a linked list
...
bool bptree_delete(bptree_t *tree, uint64_t key);
